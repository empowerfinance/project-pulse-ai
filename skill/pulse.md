...
| Pod | `/Users/thomasq/empower/project-pulse-ai/config/{name}.json` |
| BU | `/Users/thomasq/empower/project-pulse-ai/config/bu/{name}.json` — contains `teams` array referencing pod config names. Load each pod config. |
| Company | `/Users/thomasq/empower/project-pulse-ai/config/company.json` — contains `bus` array referencing BU config names. Load each BU config, which in turn loads pod configs. |
...
For each repo in the pod config (or inferred from issue branch names):
...
Save the report to `reports/` using the directory and naming conventions below.
